...
def pull_items(inData, fieldList=None, lowercasify=False):
    ...
    inputfields = [
        field.name for field in arcpy.ListFields(inData) if not field.required
    ]
    for index in range(len(inputfields)):
        if fieldList:
            if inputfields[index].upper() in [f.upper() for f in fieldList]:
                if lowercasify:
                    fieldInfo.addField(
                        inputfields[index], inputfields[index].lower(), "VISIBLE", ""
                    )
                else:
                    fieldInfo.addField(
                        inputfields[index], inputfields[index].upper(), "VISIBLE", ""
                    )
            else:
                fieldInfo.addField(inputfields[index], inputfields[index], "HIDDEN", "")
        else:
            fieldInfo.addField(inputfields[index], inputfields[index], "VISIBLE", "")
    return fieldInfo
    ...
    if not overwrite and arcpy.Exists(destPath):
        ...
        if tableOnly is True:
            arcpy.MakeTableView_management(
                sourcePath, "table_view", query, "", fieldinfo
            )
        ...
        # get spatial data
            ...
            arcpy.MakeFeatureLayer_management(sourcePath, featLyr, query, "", fieldinfo)
            ...
            if aoi and clip:
                ...
                arcpy.FeatureClassToFeatureClass_conversion(featLyr, outpath, outfile)
        # cleanup
    ...
    if not arcpy.Exists(out_fc):
        ...
        if len(roads) > 1:
            try:
                ...
                integrate_str = ";".join(
                    [r + " " + str(i + 1) for i, r in enumerate(roads)]
                )
                ...
                for i in range(1, len(roads)):
                    ...
                    arcpy.Erase_analysis(
                        roads[i], in_layer, "temp_missing_roads_" + tile, "0.01 Meters"
                    )
                    # merge the output missing roads with the previous input
                    arcpy.Merge_management(
                        ["temp_missing_roads_" + tile, in_layer], out_layer
                    )
...
@click.option(
    "--tiles", "-t", help="Comma separated list of tiles to process", default=TILES
)
def process(source_csv, n_processes, tiles):
    ...
    for t in tiles:
        fc = os.path.join(
            CONFIG["temp_data"], "tiles", "temp_" + t + ".gdb", "roads_" + t
        )
        if arcpy.Exists(fc):
            outputs = outputs + [fc]
